...
env_name = 'Hopper-v2' ; total_num_episodes = 100000
...
def main():
    ...
    num_actions_pad = 2 # numb discrete sub-actions per action dimension
    ...
    env_eval = Environment(len(env.simulator.main_data_source)-400_000, 50_000)
    ...
    print(int(env.action_space()))
    model = deepq.models.mlp_branching(
        hiddens_common=[1024*2, 1024], 
        hiddens_actions=[1024, 1024//2],  
        hiddens_value=[1024//2],
        independent=independent,
        num_action_branches=env.action_space(),
        dueling=dueling,
