...
class BranchingDQN(nn.Module): 
    ...
    def update_policy(self, adam, memory, params): 
        ...
        expected_q_vals = rewards + max_next_q_vals*0.95*masks
...
agent = BranchingDQN(env.env.observation_space(), env.env.action_space(), bins, config)
...
for frame in range(config.max_frames): 
    ...
    if np.random.random() > epsilon: 
        ...
        action = np.random.randint(0, bins, size = env.env.action_space())
