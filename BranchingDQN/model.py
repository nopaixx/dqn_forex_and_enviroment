...
class DuelingNetwork(nn.Module): 
    ...
    def __init__(self, obs, ac): 
        ...
        self.model = nn.Sequential(nn.Linear(obs, 1024*2), 
                                   nn.ReLU(), 
                                   nn.Linear(1024*2,1024*2), 
                                   nn.ReLU())

        self.value_head = nn.Linear(1024*2, 1)
        self.adv_head = nn.Linear(1024*2, ac)
...
class BranchingQNetwork(nn.Module):
    ...
    def __init__(self, obs, ac_dim, n): 
        ...
        self.value_head = nn.Linear(1024*2, 1)
        self.adv_heads = nn.ModuleList([nn.Linear(1024*2, n) for i in range(ac_dim)])
